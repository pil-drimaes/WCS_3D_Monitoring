 ...
 * 재고 정보 ETL 스케줄링 태스크
 * 
 * 재고 정보 데이터의 ETL 처리를 독립적으로 스케줄링하는 태스크
 * 
 * @author AGV Monitoring System
 * @version 2.0
 */
@Component
public class InventoryDataTask {
    
    private static final Logger log = LoggerFactory.getLogger(InventoryDataTask.class);
    
    private final InventoryDataETLEngine etlEngine;
    
    @Autowired
    public InventoryDataTask(InventoryDataETLEngine etlEngine) {
        this.etlEngine = etlEngine;
    }
    
    /**
     * 재고 정보 ETL 처리 (3초마다 실행)
     */
